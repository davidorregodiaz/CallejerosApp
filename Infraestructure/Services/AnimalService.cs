...
public class AnimalService : IAnimalService
{
    ...
    {
        var animals = await _context.Animals.Where(a => a.OwnerId == ownerId).ToListAsync();
    ...
    public async Task<TaskResult<IEnumerable<ResponseAnimalDto>>> FilterAnimalsByTypeAndBreed(string type, string breed)
{
    ...
    {
        var animalDb = await _context.Animals.FirstOrDefaultAsync(a => a.Id == id && a.OwnerId == ownerId);
        ...
        if (animalDb is not null)
        {
            ...
            if(!animalUpdated.BelongsTo(ownerId))
                return TaskResult<ResponseAnimalDto>.FromFailure("You dont have the permissions to delete this animal",403);
